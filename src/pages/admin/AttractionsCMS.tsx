...
import { Loader2, Plus, Edit2, Trash2, Save, X, Image as ImageIcon, MapPin } from "lucide-react";
...
export default function AttractionsCMS() {
      ...
      {editingItem ? (
        <div className="bg-card border border-gold/20 shadow-frame p-8">
          <div className="flex justify-between border-b border-gold/10 pb-4 mb-6">
            <h2 className="font-display text-3xl">{editingItem.id ? "Edit Attraction" : "New Attraction"}</h2>
          ...
          <form onSubmit={handleSave} className="grid lg:grid-cols-2 gap-12">
            <div className="space-y-6">
               <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    <label className="font-serif-sc text-[10px] tracking-widest text-muted-foreground block mb-2">ATTRACTION TITLE</label>
               ...
               <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="font-serif-sc text-[10px] tracking-widest text-muted-foreground block mb-2">GOOGLE MAPS LINK</label>
                    <input type="text" value={editingItem.map_link || ""} onChange={e => setEditingItem({ ...editingItem, map_link: e.target.value })} className="w-full bg-background border border-gold/20 focus:border-gold px-4 py-3 font-serif outline-none" />
                  </div>
               </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {items.map(item => (
            <div key={item.id} className="bg-card border border-gold/20 group hover:border-gold/50 transition-all shadow-frame overflow-hidden">
              ...
              <div className="p-6">
                <div className="font-serif-sc text-gold text-[10px] tracking-widest mb-2 uppercase">{item.location}</div>
                ...
                <p className="font-serif text-xs text-muted-foreground/80 line-clamp-2 italic">{item.short_description || item.full_description}</p>
              </div>
            </div>
          ))}
        </div>
